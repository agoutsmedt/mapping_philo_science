#' ---
#' title: "Script for cleaning the data"
...
#' # Cleaning the data
#' 
#' In the OST database, only the articles, books and book chapters that are listed in
#' the citing documents table have a unique identifier in the cited documents table.
#' It means that many books have no identifier (more precisely, there is one identifier
#' per citation, meaning that the same book will have as many identifiers as the 
#' number of times it is cited). Thus, they will be excluded from the coupling 
#' and co-citation analysis, which corresponds to a large loss of information (40% 
#' of citations have no proper identifier).
#' 
#' This part of the script aims at building consistent identifiers for those references
#' which lacks one.
#' 
#' ## Cleaning titles and authors
#' 
...
#' We create a new column to save the original column, just in a case of problems
#' when we clean the names.
...
#' When we have two dashes, we want to keep the first letter of the first and the second
#' name.
...
#' We now transformed `cleaned_cited_author` using the `post_dash` column that 
#' we have cleaned.
#' 

direct_citations[, cleaned_cited_author := paste0(str_remove(cleaned_cited_author, "\\-.*"),post_dash)]

#' Some special cases remains (around 3000, gathering things with one or two
#' dash that we have no cleaned properly, or things with three dashes). But as it
#' is minor cases (some names of institutions, some unusual names, etc.) it should
#' not impact what this cleaning aimed at: standardizing names to build common 
#' identifiers.
#' 
#' ## Building the identifiers
#' 
#' The strategy is to consider that:
#' 
#' 1) ref with the same title, same date and the same
#' author before the dash are the same ref. 
...
#' The goal is to build a dt with former and new ID after having implemented
#' the three different methods.

#' ### Method 1: ref with the same title, same date and the same author before the dash
...
#' We build a simple test to verify that a former ID is attributed to only one
#' new ID.
#' 
test_dt <- method_1 %>% 
  group_by(UID_Ref) %>% 
...
#' stop words before reconstituting titles. We keep only the titles with at least
#' three words because it would be too risky otherwise. 
...
#' We can now bind the results for the three methods, and check that there is 
#' no inconsistency, like 1) different `new_id` attributed to the same `UID_Ref` 
#' or 2) a `new_id` that is also a `UID_Ref` in the table.

bind_method <- rbind(method_1,method_2,method_3) %>% 
  unique() %>% 
  arrange(UID_Ref, new_ID)

#' The strategy for problem 1) is simply to remove the doublons in `UID_Ref`, as 
#' we assume that any `new_ID` is ok but we just have to keep one.

doublons <- which(duplicated(bind_method$UID_Ref))
bind_method <- bind_method[-doublons]

#' The strategy is to identify the `new_ID` which are in `UID_Ref` and to replace
#' this `new_ID` by the `new_ID` associated to the `UID_Ref`. This strategy 
#' have to be replicated until there is no more overlapping.
#' 
...
#' We can now cleaned ID in the main file, and rebuild a totally new ID from that
#' to simplify merging later (current ID are in character format because of
#' characters in the ID, and it would improve effiency to have ID in an integer
#' format).
#' 
...
#' ## Cleaning the nodes with new ID
#' 
